			...
			public void actionPerformed(ActionEvent e) {
				...
				if (num.equals("")) {
					...
					if (c == 1) {
						...
						JOptionPane.showMessageDialog(null, "编号不正确或者已超出三分钟", "消息", JOptionPane.WARNING_MESSAGE);
					}

					/*
					 * int a = OutStockDao.dellStockData(num); if (a == 0) {
				...
				if (stockNum.getText().equals("")) {
					...
					if (cmbSupName.getSelectedIndex() == 0) {
						...
						if (c == 1) {
							...
							JOptionPane.showMessageDialog(null, "请检查表号输入格式或者时间已超三分钟", "消息",
									JOptionPane.WARNING_MESSAGE);
						}

						/*
						 * // 写入 sup = (String) cmbSupName.getSelectedItem(); sun = (String)
						 * cmbStockName.getSelectedItem(); num = stockNumOut.getText(); pric =
						 * stockPricOut.getText(); ID = stockNum.getText(); user=stockUser.getText();
						 * 
						 * // 将五个值传入数据库 int a = OutStockDao.changeStockData(sup, sun, num, pric,
						 * user,ID);
						 * 
